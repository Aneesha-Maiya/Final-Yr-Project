...
@app.route('/formDisplay/getSummary', methods =["GET", "POST"])
def getSummary():
      startS = datetime.now()
    ...
    # Tokenize the input text
        startEncode = datetime.now()
        input_ids = tokenizer.encode(chunk, return_tensors="pt", max_length=1024, truncation=True)
        endEncode = datetime.now()
        tdEncode = (endEncode - startEncode).total_seconds() * 10**3
        print(f"Time taken to generate inputId's is: {tdEncode} ms ")
        print(f"Input Id's are {input_ids} and length is {len(input_ids[0])} \n")
        ...
        endGen = datetime.now()
        tdGen = (endGen - startGen).total_seconds() * 10**3
        print(f"Time taken to generate summaryId's is: {tdGen} ms ")
        print(f"Summary Id's are {summary_ids} and length is {len(summary_ids[0])}\n")

        # Decode the generated summary
        startDecode = datetime.now()
        summary = tokenizer.decode(summary_ids[0], skip_special_tokens=True)
        endDecode = datetime.now()
        tdDecode = (endDecode - startDecode).total_seconds() * 10**3
        print(f"Time taken to decode summaryId's is: {tdDecode} ms ")
